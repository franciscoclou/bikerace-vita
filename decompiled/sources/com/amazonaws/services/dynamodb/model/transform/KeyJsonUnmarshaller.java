package com.amazonaws.services.dynamodb.model.transform;

import com.amazonaws.e.c;
import com.amazonaws.e.t;
import com.amazonaws.services.dynamodb.model.Key;
import org.codehaus.jackson.JsonToken;

/* JADX INFO: loaded from: /home/francisco/bike_race/apk/classes.dex */
public class KeyJsonUnmarshaller implements t<Key, c> {
    private static KeyJsonUnmarshaller instance;

    public static KeyJsonUnmarshaller getInstance() {
        if (instance == null) {
            instance = new KeyJsonUnmarshaller();
        }
        return instance;
    }

    @Override // com.amazonaws.e.t
    public Key unmarshall(c cVar) {
        Key key = new Key();
        int iA = cVar.a();
        int i = iA + 1;
        JsonToken jsonTokenC = cVar.f98a;
        if (jsonTokenC == null) {
            jsonTokenC = cVar.c();
        }
        while (jsonTokenC != null) {
            if (jsonTokenC == JsonToken.FIELD_NAME || jsonTokenC == JsonToken.START_OBJECT) {
                if (cVar.a("HashKeyElement", i)) {
                    cVar.c();
                    key.setHashKeyElement(AttributeValueJsonUnmarshaller.getInstance().unmarshall(cVar));
                }
                if (cVar.a("RangeKeyElement", i)) {
                    cVar.c();
                    key.setRangeKeyElement(AttributeValueJsonUnmarshaller.getInstance().unmarshall(cVar));
                }
            } else if ((jsonTokenC == JsonToken.END_ARRAY || jsonTokenC == JsonToken.END_OBJECT) && cVar.a() <= iA) {
                break;
            }
            jsonTokenC = cVar.c();
        }
        return key;
    }
}
